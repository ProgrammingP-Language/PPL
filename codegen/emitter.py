...
class Emitter:
    ...
    def emit_struct_class(self, decl) -> None:
        lines = []
        lines.append(self.jvm.emitSOURCE(decl.name + ".java"))
        lines.append(self.jvm.emitCLASS("public " + decl.name))
        lines.append(self.jvm.emitSUPER("java/lang/Object"))
        lines.append("\n")

        # Khai báo các field
        for m in decl.members:
            jvm_type = self.get_jvm_type(m.member_type)
            lines.append(f".field public {m.name} {jvm_type}\n")

        # Tính stack size cần thiết cho constructor
        stack_size = max(3, 2 + sum(
            1 if (is_int_type(m.member_type) or is_float_type(m.member_type) or is_string_type(m.member_type))
            else 2  # struct field: cần new + dup
            for m in decl.members
        ) if decl.members else 2)

        # Constructor mặc định
        lines.append("\n.method public <init>()V\n")
        lines.append(f"    .limit stack {stack_size}\n")
        ...
        for m in decl.members:
            lines.append(self._emit_default_field(decl.name, m))

        lines.append("    return\n")
        lines.append(".end method\n")

        # Ghi ra file .j trong thư mục runtime
        filepath = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "runtime", f"{decl.name}.j"
        )
        with open(filepath, "w") as f:
            f.write("".join(lines))

    def _emit_default_field(self, struct_name: str, m) -> str:
        """
        Sinh bytecode khởi tạo giá trị mặc định cho 1 field của struct.

        Args:
            struct_name: Tên struct chứa field
            m: MemberDecl node của field cần khởi tạo

        Returns:
            Chuỗi bytecode khởi tạo field
        """
        mt = m.member_type
        jvm_type = self.get_jvm_type(mt)
        code = "    aload_0\n"

        if is_int_type(mt):
            code += "    iconst_0\n"
        elif is_float_type(mt):
            code += "    fconst_0\n"
        elif is_string_type(mt):
            code += '    ldc ""\n'
        elif is_struct_type(mt):
            inner_name = mt.struct_name
            code += f"    new {inner_name}\n"
            code += "    dup\n"
            code += f"    invokespecial {inner_name}/<init>()V\n"

        code += f"    putfield {struct_name}/{m.name} {jvm_type}\n"
        return code
